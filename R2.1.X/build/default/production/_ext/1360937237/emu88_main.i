...
static void setup_msdos(void) {
 ...
 flg = load_program(buf, 0xff500, &size);
 if (!flg) {
...
static int chk_os(void)
{
 ...
 while (f_readdir(&fsdir, &fileinfo) == FR_OK && fileinfo.fname[0] != 0) {
  ...
  if (strcmp(fileinfo.fname, msdosdir) == 0) {
   selection |= 2;
   printf("Detect %s\n\r", fileinfo.fname);
  }
 }
